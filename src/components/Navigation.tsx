...
export function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 50); // Show effects after 50px
    };
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden mt-2 pb-4 flex flex-col gap-4 border-t border-amber-500/20 pt-4 transition-all duration-300">
          {navItems.map((item) => (
            <a
              key={item.name}
              href={item.href}
